...
└──────────────────────────┬──────────────────────────────────────────┘
                           ...
                           ▼
┌─────────────────────────────────────────────────────────────────────┐
│  OPENCLAW — THE OPERATOR                           Always on (VPS)  │
│  1 instance per client · runs 24/7                                  │
│                                                                     │
│  ┌──────────────┐ ┌───────────────┐ ┌────────────┐ ┌─────────────┐ │
│  │    Cron       │ │    Agent      │ │  Session   │ │   Channel   │ │
│  │  scheduler    │ │  reasoning    │ │  memory    │ │  delivery   │ │
│  │              │ │               │ │            │ │             │ │
│  │ Briefings    │ │ Reads context │ │ Leads      │ │ Telegram    │ │
│  │ EOD summary  │ │ Runs workflows│ │ Clients    │ │ WhatsApp    │ │
│  │ Weekly review│ │ Decides, acts │ │ Logs, state│ │ Slack, email│ │
│  └──────────────┘ └───────────────┘ └────────────┘ └─────────────┘ │
│                                                                     │
│  The thing clients experience as "their AI"                         │
└───────┬──────▲────────────────────────────────┬──────▲──────────────┘
        │      │                                │      │
        ▼      │                                ▼      │
┌──────────────────────────────┐  ┌──────────────────────────────────┐
...
│  • Embed docs      │  │    all clients     │  │  OpenClaw reads   │
...
| OpenClaw | 1 dedicated agent instance on VPS |
...
n8n catches webhook
    ...
    │
    ▼
n8n calls OpenClaw /hooks/wake
    │── "Payment failed for Client X, £2,400, invoice #1234"
    │
    ▼
OpenClaw agent wakes up
...
Git commit + push to client's repo
    │
    ▼
OpenClaw picks up changes on next task run
    │
    ▼
...
## Two Patterns: n8n ↔ OpenClaw

### Pattern A — n8n triggers OpenClaw

External event → n8n catches, verifies, extracts → calls OpenClaw webhook with clean summary → OpenClaw reasons and acts.

**Use when:** Something happens in the outside world that the AI needs to know about.

**Examples:**
- Stripe `payment_intent.failed` → n8n extracts customer + amount → OpenClaw runs chase workflow
- Typeform submission → n8n extracts lead details → OpenClaw runs lead response workflow
- Calendar event created → n8n extracts meeting details → OpenClaw preps briefing notes

### Pattern B — OpenClaw triggers n8n

OpenClaw decides an action is needed → calls n8n webhook to execute → n8n uses stored credentials to make API calls.

**Use when:** The AI has decided what to do and needs to act on an external service.

**Examples:**
- OpenClaw drafts chase email → calls n8n → n8n sends via Gmail API
- OpenClaw flags overdue invoice → calls n8n → n8n creates follow-up task in CRM
- OpenClaw generates report → calls n8n → n8n uploads to client's Google Drive
...
Client creates a proposal
    ...
    └──▶ Text is chunked and embedded into Pinecone (AI recall only)
             │
             └──▶ When OpenClaw needs historical context:
...
| 3. Integration | n8n (credentials, webhooks) + OpenClaw (MCP, tools) |
| 4. Intelligence | OpenClaw agent (Claude model at runtime) |
| 5. Automation | OpenClaw cron (scheduled) + n8n (event-driven triggers) |
| 6. Security | OpenClaw rules + n8n credential isolation + per-client isolation |
| 7. Observability | Git repo `logs/` + OpenClaw session logs |
...
- Stand up OpenClaw instance on VPS, load context, set cron schedule
...
- Document the OpenClaw ↔ template mapping
